...
function getNowSP_localFallback() {
  ...
  return { date, minutes, source: 'local' };
}
...
export default function ClientArea({ user, onLogout }) {
  ...
  const uiAlert = async (key, variant = 'info', params = {}) => {
    if (feedback?.showMessage) {
      feedback.showMessage(key, { variant, ...params });
      return;
    }
  };
  ...
  const [serverNow, setServerNow] = useState(() => getNowSP_localFallback());
  ...
  const fetchNow = () => {
    const now = getNowSP_localFallback();
    setServerNow(now);
    return now;
  };
  ...
  const montarAgendamentosComMaps = async (agList) => {
    ...
    if (profIds.length) {
      ...
      negocioIds = uniq(profs.map(p => p.negocio_id));
      for (const p of profs) profissionaisMap.set(p.id, p);
    }
    ...
    if (negocioIds.length) {
      ...
      for (const n of (negData || [])) negociosMap.set(n.id, n);
    }
  ...
  const autoConcluirSePassou = async (agRaw, now) => {
    const toUpdate = [];
    ...
    return updateResults.some(r => r.status === 'fulfilled');
  };

  const loadData = async ({ forceNow } = {}) => {
    if (!user?.id) return;
    ...
    try {
      ...
      let agRaw = await fetchAgendamentosRaw();

      const alterou = await autoConcluirSePassou(agRaw, now);
      if (alterou) agRaw = await fetchAgendamentosRaw();
      ...
      if (favProfIds.length) {
        const { data: profFavData, error: profFavErr } = await supabase
          .from('profissionais')
          .select('id, nome, negocio_id, ativo, avatar_path')
          .in('id', favProfIds);

        if (profFavErr) throw profFavErr;

        const pList = profFavData || [];
        favNegFromProf = uniq(pList.map(p => p.negocio_id));
        for (const p of pList) favProfMap.set(p.id, p);
      }
      ...
      if (allFavNegIds.length) {
        const { data: negFavData, error: negFavErr } = await supabase
          .from('negocios')
          .select('id, nome, slug, logo_path, tipo_negocio')
          .in('id', allFavNegIds);
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      const now = fetchNow();
      if (!mounted) return;
      await loadData({ forceNow: now });
    };

    if (user?.id) boot();

    const reanchorId = setInterval(() => {
      if (!mounted) return;
      fetchNow();
    }, 10 * 60 * 1000);
  ...
  const onPickAvatar = async (e) => {
    ...
    if (!okTypes.includes(file.type)) {
      await uiAlert('clientArea.avatar_invalid_format', 'error');
      return;
    }
    if (file.size > maxMb * 1024 * 1024) {
      await uiAlert('clientArea.avatar_too_large', 'error', { maxMb });
      return;
    }
  ...
  const salvarEmail = async () => {
    ...
    if (!email || !email.includes('@')) {
      await uiAlert('clientArea.account_email_invalid', 'error');
      return;
    }
  ...
  const salvarSenha = async () => {
    ...
    if (pass.length < 6) {
      await uiAlert('clientArea.account_password_too_short', 'error');
      return;
    }
    if (pass !== conf) {
      await uiAlert('clientArea.account_password_mismatch', 'error');
      return;
    }
      ...
      <div className="mb-6">
        ...
        <div className="space-y-4">
          {lista.map(agendamento => (
            <div key={agendamento.id} className="bg-dark-200 border border-gray-800 rounded-custom p-4 sm:p-5">
              <div className="flex items-start justify-between gap-4 mb-4">
                <div className="flex-1 min-w-0">
                  <h3 className="text-lg font-normal text-white mb-1">
                    {agendamento.profissionais?.negocios?.nome || '—'}
                  </h3>
                ...
                <div className={`shrink-0 inline-flex px-3 py-1 rounded-button text-xs border ${getStatusColor(agendamento.status)}`}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 mb-4">
                <div>
                  ...
                  <div className="text-sm text-white">{formatDateBRFromISO(agendamento.data)}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 mb-1">HORÁRIO</div>
                  <div className="text-sm text-white">{agendamento.hora_inicio || '—'}</div>
                </div>
                <div>
          ...
          <div className="flex justify-between items-center h-16 sm:h-20">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="h-9 px-5 bg-dark-200 border border-gray-800 hover:border-primary/50 rounded-button text-sm transition-all uppercase focus:outline-none focus:ring-0 focus:ring-offset-0"
              >
              ...
              <button
                onClick={onLogout}
                className="h-9 flex items-center gap-2 px-5 bg-red-600 hover:bg-red-700 text-white rounded-button text-sm transition-all uppercase focus:outline-none focus:ring-0 focus:ring-offset-0"
              >
                ...
                {favoritos.length > 0 ? (
                  <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {favoritos.map(favorito => {
                      const isNegocio = favorito.tipo === 'negocio';

                      const nomeFav = isNegocio
                        ? (favorito.negocios?.nome || '—')
                        : (favorito.profissionais?.nome || '—');

                      const slug = isNegocio
                        ? favorito.negocios?.slug
                        : favorito.profissionais?.negocios?.slug;

                      const tipoNegocio = isNegocio
                        ? (String(favorito.negocios?.tipo_negocio || '').trim() || '—')
                        : 'PROFISSIONAL';
