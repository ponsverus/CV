...
export default function Dashboard({ user, onLogout }) {
  ...
  const faturamentoPorProfissionalPeriodo = useMemo(() => { const arr = metricsPeriodoData?.period?.por_profissional || []; if (!Array.isArray(arr)) return []; return arr.map(x => { if (!x) return null; const nome = String(x.nome ?? '').trim(); const valor = Number(x.faturamento ?? x.valor ?? 0); if (!nome) return null; return [nome, Number.isFinite(valor) ? valor : 0, Number(x.concluidos || 0)]; }).filter(Boolean).sort((a, b) => Number(b[1]) - Number(a[1])); }, [metricsPeriodoData]);
