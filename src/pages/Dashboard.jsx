...
function ClienteAvatar({ cliente, size = 'w-10 h-10' }) {
...
const LABEL_EMPTY_LIST       = { servicos: 'Sem serviços para este profissional.', consultas: 'Sem consultas para este profissional.', aulas: 'Sem aulas para este profissional.' };

const g = (map, group) => map[group] ?? map['servicos'];

export default function Dashboard({ user, onLogout }) {
  const feedback = useFeedback?.();
  const uiAlert   = async (key, variant = 'info') => { if (feedback?.showMessage) return feedback.showMessage(key, { variant }); };
  ...
  const fetchNowFromDb = async () => {
    ...
    setServerNow(payload); setHoje(String(payload.date)); return String(payload.date);
  };
  ...
  const extractRpcPayload = (data, key) => data?.[key] || data?.[0]?.[key] || data?.payload || data?.[0]?.payload || data;

  const loadHoje = async (negocioId) => {
    const id = negocioId || negocio?.id; if (!id) return;
    try { setMetricsHojeLoading(true); const { data, error } = await supabase.rpc('get_dashboard_today', { p_negocio_id: id }); if (error) throw error; setMetricsHoje(extractRpcPayload(data, 'get_dashboard_today')); }
    catch (e) { console.error(e); setMetricsHoje(null); } finally { setMetricsHojeLoading(false); }
  };
  ...
  const loadDia = async (negocioId, dateISO) => {
    ...
    try { setMetricsDiaLoading(true); const { data, error } = await supabase.rpc('get_dashboard_day', { p_negocio_id: id, p_date: date }); if (error) throw error; setMetricsDia(extractRpcPayload(data, 'get_dashboard_day')); }
    catch (e) { console.error(e); setMetricsDia(null); } finally { setMetricsDiaLoading(false); }
  };
  ...
  const loadPeriodo = async (negocioId, refDateISO, periodo) => {
    ...
    try { setMetricsPeriodoLoading(true); const { data, error } = await supabase.rpc('get_dashboard_period', { p_negocio_id: id, p_ref_date: refDate, p_periodo: per }); if (error) throw error; setMetricsPeriodoData(extractRpcPayload(data, 'get_dashboard_period')); }
    catch (e) { console.error(e); setMetricsPeriodoData(null); } finally { setMetricsPeriodoLoading(false); }
  };

  useEffect(() => { if (!negocio?.id || !hoje) return; loadHoje(negocio.id); loadDia(negocio.id, faturamentoData || hoje); }, [negocio?.id, hoje]);
  ...
  const fetchHistoricoPage = async ({ negocioId, profIds, date, page, append }) => {
    ...
    if (qErr) throw qErr;
    const rows = (data || []).map(a => ({ ...a, data: a?.data ?? null, horario_inicio: a?.horario_inicio ?? a?.inicio ?? null, horario_fim: a?.horario_fim ?? a?.fim ?? null }));
  ...
  const loadMoreHistorico = async () => {
    ...
    catch (e) { console.error(e); } finally { setHistoricoLoadingMore(false); }
  };

  const loadData = async (dataRef) => {
    if (!user?.id) { setError('Sessao invalida.'); setLoading(false); return; }
    setLoading(true); setError(null);
    if (dataRef === true) { try { dataRef = await fetchNowFromDb(); } catch (e) { dataRef = String(serverNow?.date || hoje || ''); } }
    try {
      const { data: negocioData, error: negocioError } = await supabase.from(VIEWS.negocios).select('*').eq('owner_id', user.id).maybeSingle();
      ...
      if (galeriaResult.error) console.error(galeriaResult.error);
      ...
      const [entregasResult, agendamentosResult] = await Promise.all([
        supabase.from(VIEWS.entregas).select('*, profissionais (id, nome)').eq('negocio_id', negocioData.id).in('profissional_id', ids).order('created_at', { ascending: false }),
        dataHoje
      ...
      setAgendamentos((agendamentosResult.data || []).map(a => ({ ...a, data: a?.data ?? null, horario_inicio: a?.horario_inicio ?? a?.inicio ?? null, horario_fim: a?.horario_fim ?? a?.fim ?? null })));
    } catch (e) { console.error(e); setError(e?.message || 'Erro inesperado.'); }
    finally { setLoading(false); }
  };

  const uploadLogoNegocio = async (file) => {
    if (!file || !user?.id || !negocio?.id) return;
    try { setLogoUploading(true); const fp = `${negocio.id}/logo.webp`; const { error: upErr } = await supabase.storage.from('logos').upload(fp, file, { upsert: true, contentType: 'image/webp' }); if (upErr) throw upErr; const { error: dbErr } = await supabase.from('negocios').update({ logo_path: `logos/${fp}` }).eq('id', negocio.id).eq('owner_id', user.id); if (dbErr) throw dbErr; await uiAlert('dashboard.logo_updated', 'success'); await loadData(true); }
    catch (e) { console.error(e); await uiAlert('dashboard.logo_update_error', 'error'); } finally { setLogoUploading(false); }
  };
  ...
  const salvarInfoNegocio = async () => {
    ...
    try {
      ...
      await uiAlert('dashboard.business_info_updated', 'success'); await loadData(true);
    } catch (e) { console.error(e); if (String(e?.message || '').includes('padrao')) await uiAlert('dashboard.address_format_invalid', 'error'); else await uiAlert('dashboard.business_info_update_error', 'error'); }
    finally { setInfoSaving(false); }
  };

  const salvarTema = async (novoTema) => {
    if (!negocio?.id) return; setFormInfo(prev => ({ ...prev, tema: novoTema }));
    try { setTemaSaving(true); const { error: updErr } = await supabase.from('negocios').update({ tema: novoTema }).eq('id', negocio.id).eq('owner_id', user.id); if (updErr) throw updErr; setNegocio(prev => prev ? { ...prev, tema: novoTema } : prev); }
    catch (e) { console.error(e); setFormInfo(prev => ({ ...prev, tema: negocio?.tema || 'dark' })); await uiAlert('dashboard.business_info_update_error', 'error'); } finally { setTemaSaving(false); }
  };

  const uploadGaleria = async (files) => {
    if (!files?.length || !negocio?.id) return;
    const maxMb = 4; const okTypes = ['image/png', 'image/jpeg', 'image/webp'];
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        const fp = `${negocio.id}/${crypto.randomUUID()}.webp`;
        const { error: upErr } = await supabase.storage.from('galerias').upload(fp, file, { contentType: 'image/webp' });
        if (upErr) { await uiAlert('dashboard.gallery_upload_error', 'error'); continue; }
        const { error: dbErr } = await supabase.from('galerias').insert({ negocio_id: negocio.id, path: `galerias/${fp}` });
        if (dbErr) await uiAlert('dashboard.gallery_upload_error', 'error');
      }
      await uiAlert('dashboard.gallery_updated', 'success');
      const { data: gd } = await supabase.from('galerias').select('id, path, ordem').eq('negocio_id', negocio.id).order('ordem', { ascending: true }).order('created_at', { ascending: true });
      setGaleriaItems(gd || []);
    } catch (e) { console.error(e); await uiAlert('dashboard.gallery_update_error', 'error'); } finally { setGalleryUploading(false); }
  };

  const removerImagemGaleria = async (item) => {
    if (!negocio?.id) return; const ok = await uiConfirm('dashboard.gallery_remove_confirm', 'warning'); if (!ok) return;
    try { await supabase.storage.from('galerias').remove([String(item.path || '').replace('galerias/', '')]); const { error: dbErr } = await supabase.from('galerias').delete().eq('id', item.id); if (dbErr) throw dbErr; setGaleriaItems(prev => prev.filter(x => x.id !== item.id)); await uiAlert('dashboard.gallery_image_removed', 'success'); }
    catch (e) { console.error(e); await uiAlert('dashboard.gallery_remove_error', 'error'); }
  };
  ...
  const createEntrega = async (e) => {
    ...
    try {
      ...
      if (preco == null) throw new Error('Preco invalido.'); if (promo != null && promo >= preco) throw new Error('Preco de oferta deve ser menor.');
      const payload = { nome: toUpperClean(formEntrega.nome), profissional_id: formEntrega.profissional_id, duracao_minutos: toNumberOrNull(formEntrega.duracao_minutos), preco, preco_promocional: promo, ativo: true, negocio_id: negocio.id };
      if (!payload.nome) throw new Error('Nome da entrega e obrigatorio.'); if (!payload.profissional_id) throw new Error('Selecione um profissional.'); if (!payload.duracao_minutos) throw new Error('Duracao invalida.');
      const { error: insErr } = await supabase.from('entregas').insert([payload]); if (insErr) throw insErr;
      await uiAlert(`dashboard.business.${businessGroup}.service_created`, 'success');
      setShowNovaEntrega(false); setEditingEntregaId(null); setFormEntrega({ nome: '', duracao_minutos: '', preco: '', preco_promocional: '', profissional_id: '' }); await loadData(true);
    } catch (e2) { console.error(e2); const msg = String(e2?.message || ''); if (msg.includes('oferta')) await uiAlert('dashboard.service_promo_invalid', 'error'); else if (msg.includes('invalido')) await uiAlert('dashboard.service_price_invalid', 'error'); else if (msg.includes('Duracao')) await uiAlert('dashboard.service_duration_invalid', 'error'); else if (msg.includes('Selecione')) await uiAlert(`dashboard.business.${businessGroup}.service_prof_required`, 'error'); else await uiAlert(`dashboard.business.${businessGroup}.service_create_error`, 'error'); }
  };
  ...
  const updateEntrega = async (e) => {
    ...
    try {
      const preco = toNumberOrNull(formEntrega.preco); const promo = toNumberOrNull(formEntrega.preco_promocional);
      if (!toUpperClean(formEntrega.nome)) throw new Error('Nome da entrega e obrigatorio.'); if (!formEntrega.profissional_id) throw new Error('Selecione um profissional.'); if (!toNumberOrNull(formEntrega.duracao_minutos)) throw new Error('Duracao invalida.'); if (preco == null) throw new Error('Preco invalido.'); if (promo != null && promo >= preco) throw new Error('Preco de oferta deve ser menor.');
      const payload = { nome: toUpperClean(formEntrega.nome), duracao_minutos: toNumberOrNull(formEntrega.duracao_minutos), preco, preco_promocional: promo, profissional_id: formEntrega.profissional_id };
      const { error: updErr } = await supabase.from('entregas').update(payload).eq('id', editingEntregaId).eq('negocio_id', negocio.id); if (updErr) throw updErr;
  ...
  const deleteEntrega = async (id) => {
    ...
    try { const { error: delErr } = await supabase.from('entregas').delete().eq('id', id).eq('negocio_id', negocio.id); if (delErr) throw delErr; await uiAlert(`dashboard.business.${businessGroup}.service_deleted`, 'success'); await loadData(true); }
    catch (e2) { console.error(e2); await uiAlert(`dashboard.business.${businessGroup}.service_delete_error`, 'error'); }
  };
  ...
  const createProfissional = async (e) => {
    ...
    try {
      ...
      const { error: insErr } = await supabase.from('profissionais').insert([payload]); if (insErr) throw insErr;
      await uiAlert('dashboard.professional_created', 'success');
      setShowNovoProfissional(false); setEditingProfissional(null); setFormProfissional({ nome: '', profissao: '', anos_experiencia: '', horario_inicio: '08:00', horario_fim: '18:00', almoco_inicio: '', almoco_fim: '', dias_trabalho: [1,2,3,4,5,6] }); await loadData(true);
    } catch (e2) { console.error(e2); await uiAlert('dashboard.professional_create_error', 'error'); }
  };
  ...
  const updateProfissional = async (e) => {
    ...
    try {
      ...
      if (!payload.nome) throw new Error('Nome e obrigatorio.');
      const { error: updErr } = await supabase.from('profissionais').update(payload).eq('id', editingProfissional.id).eq('negocio_id', negocio.id); if (updErr) throw updErr;
      await uiAlert('dashboard.professional_updated', 'success'); setShowNovoProfissional(false); setEditingProfissional(null); await loadData(true);
    } catch (e2) { console.error(e2); const msg = String(e2?.message || ''); if (msg.includes('profissional_almoco_bloqueado')) await uiAlert('dashboard.professional_lunch_blocked', 'warning'); else if (msg.includes('profissional_dia_bloqueado')) await uiAlert('dashboard.professional_workday_blocked', 'warning'); else await uiAlert('dashboard.professional_update_error', 'error'); }
  };
  ...
  const toggleAtivoProfissional = async (p) => {
    try {
      ...
      const { error: upErr } = await supabase.from('profissionais').update({ ativo: novoAtivo, motivo_inativo: novoAtivo ? null : motivo }).eq('id', p.id).eq('negocio_id', negocio.id); if (upErr) throw upErr;
      await uiAlert(novoAtivo ? 'dashboard.professional_activated' : 'dashboard.professional_inactivated', 'success'); await loadData(true);
    } catch (e) { console.error(e); await uiAlert('dashboard.professional_toggle_error', 'error'); }
  };

  const excluirProfissional = async (p) => {
    const ok = await uiConfirm('dashboard.professional_delete_confirm', 'warning'); if (!ok) return;
    try { const { error: delErr } = await supabase.from('profissionais').delete().eq('id', p.id).eq('negocio_id', negocio.id); if (delErr) throw delErr; await uiAlert('dashboard.professional_deleted', 'success'); await loadData(true); }
    catch (e) { console.error(e); await uiAlert('dashboard.professional_delete_error', 'error'); }
  };

  const confirmarAtendimento = async (id) => {
    try { const { error: updErr } = await supabase.from('agendamentos').update({ status: 'concluido' }).eq('id', id).eq('negocio_id', negocio.id); if (updErr) throw updErr; await uiAlert('dashboard.booking_confirmed', 'success'); await loadData(true); }
    catch (e2) { console.error(e2); await uiAlert('dashboard.booking_confirm_error', 'error'); }
  };
  ...
  const cancelarAgendamentoProfissional = async (id) => {
    ...
    try { const { error: updErr } = await supabase.from('agendamentos').update({ status: 'cancelado_profissional' }).eq('id', id).eq('negocio_id', negocio.id); if (updErr) throw updErr; await uiAlert('dashboard.booking_canceled', 'error'); await loadData(true); }
    catch (e) { console.error(e); await uiAlert('dashboard.booking_cancel_error', 'error'); }
  };
  ...
  const saveParceiroEmail = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('profissionais').update({ email: email.toLowerCase() }).eq('id', parceiroSelected).eq('negocio_id', negocio.id); if (error) throw error;
      setProfissionais(prev => prev.map(p => p.id === parceiroSelected ? { ...p, email: email.toLowerCase() } : p));
      setParceiroSelected(''); setParceiroEmail(''); await uiAlert('dashboard.parceiro_saved', 'success');
    } catch (e) { console.error(e); await uiAlert('dashboard.parceiro_save_error', 'error'); } finally { setParceiroSaving(false); }
  };

  const removeParceiroEmail = async (profId) => {
    const ok = await uiConfirm('dashboard.parceiro_delete_confirm', 'warning'); if (!ok) return;
    try { const { error } = await supabase.from('profissionais').update({ email: null }).eq('id', profId).eq('negocio_id', negocio.id); if (error) throw error; setProfissionais(prev => prev.map(p => p.id === profId ? { ...p, email: null } : p)); await uiAlert('dashboard.parceiro_deleted', 'success'); }
    catch (e) { console.error(e); await uiAlert('dashboard.parceiro_delete_error', 'error'); }
  };
  ...
  const salvarEmail = async () => {
    ...
    catch (e) { console.error(e); await uiAlert('dashboard.account_email_update_error', 'error'); } finally { setSavingDados(false); }
  };
  ...
  const salvarSenha = async () => {
    ...
    catch (e) { console.error(e); await uiAlert('dashboard.account_password_update_error', 'error'); } finally { setSavingDados(false); }
  };
  ...
  const agendamentosAgrupadosPorProfissional = useMemo(() => {
    ...
    const grupos = Array.from(map.values()).map(g => ({ ...g, itens: g.itens.slice().sort((a, b) => { const d = String(getAgDate(a)).localeCompare(String(getAgDate(b))); if (d !== 0) return d; const h = String(getAgInicio(a)).localeCompare(String(getAgInicio(b))); if (h !== 0) return h; return String(a.id).localeCompare(String(b.id)); }) }));
  ...
  const mkPairs = (arr) => {
    ...
    return arr.map(x => { if (!x) return null; const nome = String(x.nome ?? '').trim(); const valor = Number(x.faturamento ?? x.valor ?? 0); return nome ? [nome, Number.isFinite(valor) ? valor : 0] : null; }).filter(Boolean).sort((a, b) => Number(b[1]) - Number(a[1]));
  };
  const faturamentoPorProfissionalHoje  = useMemo(() => mkPairs(metricsHoje?.today?.por_profissional), [metricsHoje]);
  const faturamentoPorProfissionalFiltro = useMemo(() => mkPairs(metricsDia?.selected_day?.por_profissional), [metricsDia]);

  const tabs = [{ key: 'visao-geral' }, { key: 'agendamentos' }, { key: 'cancelados' }, { key: 'historico' }, { key: 'entregas' }, { key: 'profissionais' }, { key: 'info-negocio' }];
  ...
  if (loading) return (<div className="min-h-screen bg-black flex items-center justify-center"><div className="text-center"><div className="w-16 h-16 border-4 border-primary border-t-transparent rounded-full animate-spin mx-auto mb-4"></div><div className="text-primary text-xl">CARREGANDO...</div></div></div>);
  if (error || !negocio) return (<div className="min-h-screen bg-black flex items-center justify-center p-4"><div className="max-w-md w-full bg-dark-100 border border-red-500/50 rounded-custom p-8 text-center"><AlertCircle className="w-16 h-16 text-red-500 mx-auto mb-4" /><h1 className="text-2xl font-normal text-white mb-2">Erro ao carregar</h1><p className="text-gray-400 mb-6">{error || 'Negocio inexistente'}</p><button onClick={() => loadData(true)} className="w-full px-6 py-3 bg-primary/20 border border-primary/50 text-primary rounded-button mb-3 font-normal uppercase">TENTAR NOVAMENTE</button><button onClick={onLogout} className="w-full px-6 py-3 bg-red-600 hover:bg-red-700 text-white rounded-button font-normal uppercase">SAIR</button></div></div>);
  ...
  return (
    <div className="min-h-screen bg-black text-white">
          ...
          <div className="flex justify-between items-center h-20">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full overflow-hidden border border-gray-800 bg-dark-200 flex items-center justify-center shrink-0">
                {negocio.logo_path ? <img src={getPublicUrl('logos', negocio.logo_path)} alt="Logo" className="w-full h-full object-cover" /> : <div className="w-12 h-12 bg-gradient-to-br from-primary to-yellow-600 rounded-full flex items-center justify-center"><Award className="w-7 h-7 text-black" /></div>}
              </div>
              <div><h1 className="text-xl font-normal">{negocio.nome}</h1><p className="text-xs text-gray-500 -mt-0.5">DASHBOARD</p></div>
            </div>
            <div className="flex items-center gap-2 sm:gap-3">
              <Link to={`/v/${negocio.slug}`} target="_blank" className="hidden sm:flex items-center gap-2 px-4 py-2 bg-dark-200 border border-gray-800 hover:border-primary rounded-button text-sm font-normal uppercase"><Eye className="w-4 h-4" />VER VITRINE</Link>
              <label className="inline-block">
                <input type="file" accept="image/*" className="hidden" onChange={(e) => uploadLogoNegocio(e.target.files?.[0])} disabled={logoUploading} />
                <span className={`inline-flex items-center justify-center rounded-button font-normal border transition-all uppercase ${logoUploading ? 'bg-gray-900 border-gray-800 text-gray-600 cursor-not-allowed' : 'bg-primary/10 hover:bg-primary/20 border-primary/30 text-primary cursor-pointer'} px-3 py-2 text-[11px] sm:px-4 sm:py-2 sm:text-sm`}>
                  <span className="sm:hidden">{logoUploading ? '...' : 'LOGO'}</span>
                  <span className="hidden sm:inline">{logoUploading ? 'ENVIANDO...' : 'ALTERAR LOGO'}</span>
                </span>
              </label>
              <button onClick={onLogout} className="flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 rounded-button text-sm font-normal uppercase"><LogOut className="w-4 h-4" /><span className="hidden sm:inline">SAIR</span></button>
            </div>
          </div>
        </div>
      </header>
              ...
              <div key={i} className="announcement-bar-track flex items-center shrink-0 whitespace-nowrap" aria-hidden={i === 2}>
                {[...Array(12)].map((_, index) => (
                  <div key={index} className="flex items-center">
                    <span className="text-black font-bold text-sm uppercase mx-4">CLIQUE PARA IR</span><span className="text-black mx-4">●</span>
                    <Link to={`/v/${negocio.slug}`} target="_blank" className="text-black font-normal text-sm uppercase hover:underline underline-offset-4 transition-all mx-4">VER VITRINE</Link><span className="text-black mx-4">●</span>
                    <a href={SUPORTE_HREF} target="_blank" rel="noreferrer" className="text-black font-normal text-sm uppercase hover:underline underline-offset-4 transition-all mx-4">SUPORTE</a><span className="text-black mx-4">●</span>
                  </div>
                ))}
              </div>
            ))}
          </div>
          <style>{`@keyframes announcement-scroll{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}.announcement-bar-wrapper{display:flex;width:max-content;animation:announcement-scroll 50s linear infinite}.announcement-bar-wrapper:hover{animation-play-state:paused}.announcement-bar-track a{position:relative;z-index:10;cursor:pointer;display:inline-block}@media(prefers-reduced-motion:reduce){.announcement-bar-wrapper{animation:none}}`}</style>
        </div>
              ...
              <div className="space-y-6">
                <div className="grid md:grid-cols-3 gap-4">
                  ...
                  <div className="bg-dark-200 border border-gray-800 rounded-custom p-5">
                    ...
                    {proximoAgendamento ? (<><div className="text-3xl font-normal text-primary">{getAgInicio(proximoAgendamento)}</div><div className="text-sm text-gray-300 mt-1">{proximoAgendamento.cliente?.nome || '—'} • {proximoAgendamento.profissionais?.nome}</div><div className="text-xs text-gray-500 mt-1">{proximoAgendamento.entregas?.nome}</div></>) : <div className="text-sm text-gray-500">:(</div>}
                  </div>
                </div>
                {faturamentoPorProfissionalHoje.length > 0 && (<div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">{faturamentoPorProfissionalHoje.map(([nome, valor]) => (<div key={String(nome)} className="bg-dark-200 border border-gray-800 rounded-custom p-5"><div className="text-xs text-gray-500 mb-1">PROFISSIONAL</div><div className="font-normal text-white">{String(nome || '—')}</div><div className="text-primary font-normal mt-1">R$ {Number(valor || 0).toFixed(2)}</div></div>))}</div>)}
                <div className="bg-dark-200 border border-gray-800 rounded-custom p-5">
                  ...
                  {faturamentoPorProfissionalFiltro.length > 0 && (<div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">{faturamentoPorProfissionalFiltro.map(([nome, valor]) => (<div key={String(nome)} className="bg-dark-100 border border-gray-800 rounded-custom p-4"><div className="text-xs text-gray-500 mb-1">PROFISSIONAL</div><div className="font-normal text-white">{String(nome || '—')}</div><div className="text-primary font-normal mt-1">R$ {Number(valor || 0).toFixed(2)}</div></div>))}</div>)}
                      ...
                      <div key={grupo.pid} className="space-y-4">
                        ...
                        <div className="space-y-4">
                          {grupo.itens.map(a => {
                            ...
                            return (
                              <div key={a.id} className="bg-dark-200 border border-gray-800 rounded-custom p-4 sm:p-5">
                                {/* Cabeçalho: avatar + info empilhada + badge */}
                                <div className="flex items-start justify-between gap-4 mb-4">
                                  <div className="flex items-center gap-3 min-w-0">
                                    <ClienteAvatar cliente={a.cliente} />
                                    <div className="min-w-0">
                                      <p className="text-lg font-normal text-white truncate">{a.cliente?.nome || '—'}</p>
                                      <p className="text-sm text-gray-400 mt-0.5 truncate">{a.profissionais?.nome || '—'}</p>
                                      <p className="text-sm text-primary mt-0.5 truncate">{a.entregas?.nome || '—'}</p>
                                    </div>
                                  </div>
                                  <div className="shrink-0">
                                    {isCancel  ? <span className="inline-flex px-3 py-1 rounded-button text-xs border bg-red-500/20    border-red-500/50    text-red-400">CANCELADO</span>
                                    : isDone   ? <span className="inline-flex px-3 py-1 rounded-button text-xs border bg-green-500/20  border-green-500/50  text-green-400">CONCLUÍDO</span>
                                    : isFuturo ? <span className="inline-flex px-3 py-1 rounded-button text-xs border bg-yellow-500/20 border-yellow-500/50 text-yellow-300">FUTURO</span>
                                    :            <span className="inline-flex px-3 py-1 rounded-button text-xs border bg-blue-500/20   border-blue-500/50   text-blue-400">AGENDADO</span>}
                                  </div>
                                </div>
                                {/* Grid DATA / HORÁRIO / VALOR */}
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 mb-4">
                                  <div><div className="text-xs text-gray-500 mb-1">DATA</div><div className="text-sm text-white">{formatDateBRFromISO(getAgDate(a))}</div></div>
                                  <div><div className="text-xs text-gray-500 mb-1">HORÁRIO</div><div className="text-sm text-white">{getAgInicio(a)}</div></div>
                                  <div><div className="text-xs text-gray-500 mb-1">VALOR</div><div className="text-sm text-white">R$ {Number(valorReal).toFixed(2)}</div></div>
                                </div>
                                {/* Botões */}
                                {!isDone && !isCancel && (
                                  isHoje ? (
                ...
                {hojeCancelados.length > 0 ? (
                  <div className="space-y-4">
                    {hojeCancelados.sort((a, b) => String(getAgInicio(a)).localeCompare(String(getAgInicio(b)))).map(a => {
                      const valorReal = getValorAgendamento(a);
                      return (
                        <div key={a.id} className="bg-dark-200 border border-red-500/30 rounded-custom p-4 sm:p-5">
                          <div className="flex items-start justify-between gap-4 mb-4">
                            <div className="flex items-center gap-3 min-w-0">
                              <ClienteAvatar cliente={a.cliente} />
                              <div className="min-w-0">
                            ...
                            <span className="shrink-0 inline-flex px-3 py-1 rounded-button text-xs border bg-red-500/20 border-red-500/50 text-red-400">CANCELADO</span>
                          </div>
                ...
                {agendamentosDiaSelecionado.length > 0 ? (
                  <div className="space-y-4">
                    {agendamentosDiaSelecionado.map(a => {
                      ...
                      const valorReal = getValorAgendamento(a);
                      return (
                        <div key={a.id} className={`bg-dark-200 border rounded-custom p-4 sm:p-5 ${isCancel ? 'border-red-500/30' : 'border-gray-800'}`}>
                          <div className="flex items-start justify-between gap-4 mb-4">
                            <div className="flex items-center gap-3 min-w-0">
                              <ClienteAvatar cliente={a.cliente} />
                              <div className="min-w-0">
            ...
            {activeTab === 'entregas' && (
              <div>
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button onClick={() => { setShowNovaEntrega(true); setEditingEntregaId(null); setFormEntrega({ nome: '', duracao_minutos: '', preco: '', preco_promocional: '', profissional_id: '' }); }} className="flex items-center gap-2 px-5 py-2.5 rounded-button font-normal uppercase border bg-gradient-to-r from-primary to-yellow-600 text-black border-transparent"><Plus className="w-5 h-5" />{btnAddLabel}</button>
                </div>
                                ...
                                return (
                                  <div key={s.id} className="bg-dark-100 border border-gray-800 rounded-custom p-5">
                                    <div className="flex justify-between items-start mb-3">
                                      <div><h3 className="text-lg font-normal">{s.nome}</h3><p className="text-xs text-gray-500">{p.nome}</p></div>
                                      <div className="text-right">{promo != null && promo > 0 && promo < preco ? (<div className="leading-tight"><div className="text-sm font-normal text-red-400 line-through">R$ {preco.toFixed(2)}</div><div className="text-2xl font-normal text-green-400">R$ {promo.toFixed(2)}</div></div>) : (<div className="text-2xl font-normal text-primary">R$ {preco.toFixed(2)}</div>)}</div>
                                    </div>
            ...
            {activeTab === 'profissionais' && (
              <div>
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button onClick={() => { setShowNovoProfissional(true); setEditingProfissional(null); setFormProfissional({ nome: '', profissao: '', anos_experiencia: '', horario_inicio: '08:00', horario_fim: '18:00', almoco_inicio: '', almoco_fim: '', dias_trabalho: [1,2,3,4,5,6] }); }} className="flex items-center gap-2 px-5 py-2.5 rounded-button font-normal uppercase border bg-gradient-to-r from-primary to-yellow-600 text-black border-transparent"><Plus className="w-5 h-5" />ADICIONAR</button>
                </div>
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {profissionais.map(p => {
                    const ativo = (p.ativo === undefined) ? true : !!p.ativo;
                    const dotClass = STATUS_COLOR_CLASS[normalizeKey(p.status_label)] || 'bg-gray-500';
                    return (
                      <div key={p.id} className="relative bg-dark-200 border border-gray-800 rounded-custom p-5">
                        <div className="flex items-start gap-3 mb-3">
                          <div className="w-12 h-12 bg-gradient-to-br from-primary to-yellow-600 rounded-custom flex items-center justify-center text-xl font-normal shrink-0">{p.nome?.[0] || 'P'}</div>
                          <div className="flex-1 min-w-0">
                            <h3 className="font-normal">{p.nome}{!ativo && <span className="absolute top-3 right-3 text-[10px] px-3 py-1 rounded-full bg-red-500/10 border border-red-500/20 text-red-300 font-normal uppercase">INATIVO</span>}</h3>
                            <div className="flex items-center gap-2 mt-1"><span className={`w-2.5 h-2.5 rounded-full shrink-0 ${dotClass}`} /><span className="text-xs text-gray-400">{p.status_label || '—'}</span></div>
                        ...
                        <div className="text-xs text-gray-500 mb-3"><Clock className="w-4 h-4 inline mr-1" />{p.horario_inicio} - {p.horario_fim}{p.almoco_inicio && p.almoco_fim && <span className="ml-2 text-yellow-300">• {p.almoco_inicio} - {p.almoco_fim}</span>}</div>
                        <div className="flex gap-2 mb-3">
                          <button onClick={() => toggleAtivoProfissional(p)} className={`flex-1 py-2 rounded-button text-sm border font-normal uppercase ${ativo ? 'bg-yellow-500/10 border-yellow-500/30 text-yellow-300' : 'bg-green-500/10 border-green-500/30 text-green-300'}`}>{ativo ? 'INATIVAR' : 'ATIVAR'}</button>
            ...
            {activeTab === 'info-negocio' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button onClick={salvarInfoNegocio} disabled={infoSaving} className={`px-5 py-2.5 rounded-button font-normal border flex items-center gap-2 uppercase ${infoSaving ? 'bg-gray-900 border-gray-800 text-gray-600 cursor-not-allowed' : 'bg-primary/20 hover:bg-primary/30 border-primary/50 text-primary'}`}><Save className="w-4 h-4" />{infoSaving ? 'SALVANDO...' : 'SALVAR'}</button>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-dark-200 border border-gray-800 rounded-custom p-5"><label className="block text-sm mb-2">Instagram</label><input value={formInfo.instagram} onChange={(e) => setFormInfo(prev => ({ ...prev, instagram: e.target.value }))} className="w-full px-4 py-3 bg-dark-100 border border-gray-800 rounded-custom text-white" placeholder="@seuinstagram" /></div>
                  <div className="bg-dark-200 border border-gray-800 rounded-custom p-5"><label className="block text-sm mb-2">Facebook</label><input value={formInfo.facebook} onChange={(e) => setFormInfo(prev => ({ ...prev, facebook: e.target.value }))} className="w-full px-4 py-3 bg-dark-100 border border-gray-800 rounded-custom text-white" placeholder="facebook.com/..." /></div>
                </div>
                <div className="bg-dark-200 border border-gray-800 rounded-custom p-6">
                  <div className="text-sm tracking-wide mb-1">Parceiros</div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-3 mb-3">
                    <div><label className="block text-xs text-gray-400 mb-1">PROFISSIONAL</label><ProfissionalSelect value={parceiroSelected} onChange={(id) => { setParceiroSelected(id); setParceiroEmail(profissionais.find(p => p.id === id)?.email || ''); }} profissionais={profissionais} placeholder="Selecionar profissional..." apenasAtivos={true} /></div>
                    <div><label className="block text-xs text-gray-400 mb-1">EMAIL DO PARCEIRO</label><input type="email" value={parceiroEmail} onChange={(e) => setParceiroEmail(e.target.value)} className="w-full px-4 py-3 bg-dark-100 border border-gray-800 rounded-custom text-white" placeholder="parceiro@email.com" /></div>
                  </div>
                  <button type="button" disabled={parceiroSaving || !parceiroSelected || !parceiroEmail.trim()} onClick={saveParceiroEmail} className="mb-5 px-5 py-2 bg-primary/20 hover:bg-primary/30 border border-primary/50 text-primary rounded-button text-sm font-normal uppercase disabled:opacity-50 disabled:cursor-not-allowed">{parceiroSaving ? 'SALVANDO...' : 'SALVAR E-MAIL'}</button>
                  {profissionais.filter(p => p.email).length > 0 ? (
                    <div className="space-y-2">{profissionais.filter(p => p.email).map(p => (<div key={p.id} className="flex items-center justify-between bg-dark-100 border border-gray-800 rounded-custom px-4 py-3"><div><div className="text-sm font-normal text-white">{p.nome}</div><div className="text-xs text-gray-500">{p.email}</div></div><button onClick={() => removeParceiroEmail(p.id)} className="text-red-400 hover:text-red-300 text-xs uppercase font-normal border border-red-500/30 rounded-button px-3 py-1">REMOVER</button></div>))}</div>
                ...
                <div className="bg-dark-200 border border-gray-800 rounded-custom p-6">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div><label className="block text-sm mb-2">EMAIL</label><input type="email" value={novoEmail} onChange={(e) => setNovoEmail(e.target.value)} className="w-full px-4 py-3 bg-dark-100 border border-gray-800 rounded-custom text-white" placeholder="seu@email.com" /><button type="button" disabled={savingDados} onClick={salvarEmail} className="mt-3 w-full py-2 bg-primary/20 hover:bg-primary/30 border border-primary/50 text-primary rounded-button text-sm disabled:opacity-60 disabled:cursor-not-allowed uppercase font-normal">SALVAR NOVO EMAIL</button></div>
                    <div><label className="block text-sm mb-2">NOVA SENHA</label><input type="password" value={novaSenha} onChange={(e) => setNovaSenha(e.target.value)} className="w-full px-4 py-3 bg-dark-100 border border-gray-800 rounded-custom text-white" placeholder="••••••••" /><label className="block text-sm mb-2 mt-3">CONFIRMAR SENHA</label><input type="password" value={confirmarSenha} onChange={(e) => setConfirmarSenha(e.target.value)} className="w-full px-4 py-3 bg-dark-100 border border-gray-800 rounded-custom text-white" placeholder="••••••••" /><button type="button" disabled={savingDados} onClick={salvarSenha} className="mt-3 w-full py-2 bg-green-500/20 hover:bg-green-500/30 border border-green-500/50 text-green-300 rounded-button text-sm disabled:opacity-60 disabled:cursor-not-allowed uppercase font-normal">SALVAR NOVA SENHA</button></div>
                  </div>
                </div>
                <div className="bg-dark-200 border border-gray-800 rounded-custom p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-normal">GALERIA</h3>
                    <label className="inline-block"><input type="file" accept="image/png,image/jpeg,image/webp" multiple className="hidden" onChange={(e) => uploadGaleria(e.target.files)} disabled={galleryUploading} /><span className={`inline-flex items-center gap-2 rounded-button font-normal border cursor-pointer transition-all uppercase ${galleryUploading ? 'bg-gray-900 border-gray-800 text-gray-600 cursor-not-allowed' : 'bg-primary/20 hover:bg-primary/30 border-primary/50 text-primary'} px-3 py-1.5 text-xs sm:px-4 sm:py-2 sm:text-sm`}><Plus className="w-4 h-4" />{galleryUploading ? 'ENVIANDO...' : 'ADICIONAR'}</span></label>
                  </div>
                  {galeriaItems.length > 0 ? (
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">{galeriaItems.map((item) => (<div key={item.id || item.path} className="relative bg-dark-100 border border-gray-800 rounded-custom overflow-hidden"><img src={getPublicUrl('galerias', item.path)} alt="Galeria" className="w-full h-28 object-cover" /><button onClick={() => removerImagemGaleria(item)} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 sm:left-2 sm:top-2 sm:right-auto sm:transform-none px-3 py-1 rounded-full bg-black/60 border border-gray-700 hover:border-red-400 text-[12px] text-red-200 font-normal uppercase">REMOVER</button></div>))}</div>
      ...
      {showNovaEntrega && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
          <div className="bg-dark-100 border border-gray-800 rounded-custom max-w-md w-full p-8 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6"><h3 className="text-2xl font-normal">{editingEntregaId ? modalEditLabel : modalNewLabel}</h3><button onClick={() => { setShowNovaEntrega(false); setEditingEntregaId(null); setFormEntrega({ nome: '', duracao_minutos: '', preco: '', preco_promocional: '', profissional_id: '' }); }}><X className="w-6 h-6" /></button></div>
            <form onSubmit={editingEntregaId ? updateEntrega : createEntrega} className="space-y-4">
              <div><label className="block text-sm mb-2">Profissional</label><ProfissionalSelect value={formEntrega.profissional_id} onChange={(id) => setFormEntrega({ ...formEntrega, profissional_id: id })} profissionais={profissionais} placeholder="Selecione" apenasAtivos={true} /></div>
              ...
              <div><label className="block text-sm mb-2">Preço de OFERTA (opcional)</label><input type="number" step="0.01" value={formEntrega.preco_promocional} onChange={(e) => setFormEntrega({ ...formEntrega, preco_promocional: e.target.value })} className="w-full px-4 py-3 bg-dark-200 border border-gray-800 rounded-custom text-white" placeholder="Apenas se houver oferta" /><p className="text-[12px] text-gray-500 mt-2">O preço de oferta deve ser menor que o preço normal.</p></div>
              <button type="submit" className="w-full py-3 bg-gradient-to-r from-primary to-yellow-600 text-black rounded-button font-normal uppercase">{editingEntregaId ? 'SALVAR' : `CRIAR ${btnAddLabel}`}</button>
            </form>
          </div>
        </div>
      )}
      ...
      {showNovoProfissional && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
          <div className="bg-dark-100 border border-gray-800 rounded-custom max-w-md w-full p-8 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6"><h3 className="text-2xl font-normal">{editingProfissional ? 'EDITAR PROFISSIONAL' : 'NOVO PROFISSIONAL'}</h3><button onClick={() => { setShowNovoProfissional(false); setEditingProfissional(null); setFormProfissional({ nome: '', profissao: '', anos_experiencia: '', horario_inicio: '08:00', horario_fim: '18:00', almoco_inicio: '', almoco_fim: '', dias_trabalho: [1,2,3,4,5,6] }); }}><X className="w-6 h-6" /></button></div>
            <form onSubmit={editingProfissional ? updateProfissional : createProfissional} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm mb-2">Das</label><TimePicker value={formProfissional.horario_inicio} onChange={(v) => setFormProfissional({ ...formProfissional, horario_inicio: v })} step={30} /></div><div><label className="block text-sm mb-2">Até</label><TimePicker value={formProfissional.horario_fim} onChange={(v) => setFormProfissional({ ...formProfissional, horario_fim: v })} step={30} /></div></div>
              <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm mb-2">Almoço (Início)</label><TimePicker value={formProfissional.almoco_inicio} onChange={(v) => setFormProfissional({ ...formProfissional, almoco_inicio: v })} step={15} /></div><div><label className="block text-sm mb-2">Almoço (Fim)</label><TimePicker value={formProfissional.almoco_fim} onChange={(v) => setFormProfissional({ ...formProfissional, almoco_fim: v })} step={15} /></div></div>
              <div>
                <label className="block text-sm mb-2">Dias de trabalho</label>
                <div className="grid grid-cols-5 sm:grid-cols-7 gap-2">
                  {WEEKDAYS.map(d => { const active = (formProfissional.dias_trabalho || []).includes(d.i); return (<button type="button" key={d.i} onClick={() => { const cur = Array.isArray(formProfissional.dias_trabalho) ? [...formProfissional.dias_trabalho] : []; const next = active ? cur.filter(x => x !== d.i) : [...cur, d.i]; setFormProfissional(prev => ({ ...prev, dias_trabalho: normalizeDiasTrabalho(next) })); }} className={`py-2 rounded-button border font-normal text-xs transition-all ${active ? 'bg-primary/20 border-primary/50 text-primary' : 'bg-dark-200 border-gray-800 text-gray-500'}`}>{d.label}</button>); })}
                </div>
              </div>
              <button type="submit" className="w-full py-3 bg-gradient-to-r from-primary to-yellow-600 text-black rounded-button font-normal uppercase">{editingProfissional ? 'SALVAR' : 'ADICIONAR'}</button>
            </form>
          </div>
        </div>
      )}
